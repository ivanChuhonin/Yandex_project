...
Модульная версия [исходного ноутбука](https://github.com/ivanChuhonin/Yandex_project/tree/main) — с исправленными багами, единой логикой разметки тональности и воспроизводимым пайплайном.
...
Для сравнения на том же test-split прогнан zero-shot `blanchefort/rubert-base-cased-sentiment` — результат практически идентичный (accuracy 0.88, macro F1 0.62, neutral F1 0.27). Более тяжёлая предобученная модель не дала прироста: `neutral` — не проблема выбора модели, а следствие того, что 3-звёздочные отзывы текстуально пограничны между позитивом и негативом (ошибки в обоих подходах делятся примерно поровну между "ушёл в negative" и "ушёл в positive").
...
- Бизнес-анализ (фастфуд/маркетплейсы/супермаркеты по городам) считается на полном датасете, а не после фильтрации по рубрике "Ресторан/Кафе".
- `RandomizedSearchCV` используется для подбора гиперпараметров.
